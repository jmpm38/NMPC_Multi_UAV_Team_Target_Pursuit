...
class one_opt:
    ...
    def __init__(self):
        ...
        # Initialize ROS
        rospy.init_node('uav_publisher_subscriber', anonymous=True) # Anonymous makes it so that multiple listeners can be run simultaneously, ensuring different names
...
###################################################################################
    ...
    def NMPC_logic(self):
        if((rospy.get_time() - self.start_timer) > self.sim_time):
            ...
            # rospy.loginfo("UAV STATE: " + str(self.uav_state))
            ...
            # print("optimized: " + str(u_optimized[3]))
            self.yaw = ((u_optimized[3] + np.pi) % (2*np.pi))-np.pi
            # print("ROLL: " + str(self.roll))
            # print("PITCH: " + str(self.pitch))
            # print("YAW: " + str(self.yaw))

            # rospy.loginfo("INPUTS: [" + str(np.around(self.throttle, 3)) + ", " + str(np.around(self.roll, 3)) + ", " + str(np.around(self.pitch, 3)) + ", " + str(np.around(self.yaw, 3)) + "]")
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if (self.data):
                ...
                if((self.iteration_counter > 0) and self.not_cmd_kill):
