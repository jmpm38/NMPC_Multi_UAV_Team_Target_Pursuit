...
class one_opt:
    ...
    def __init__(self):
        ...
        # Initialize ROS
        rospy.init_node('uav3_publisher_subscriber', anonymous=True) # Anonymous makes it so that multiple listeners can be run simultaneously, ensuring different names
        self.pub = rospy.Publisher('/uav3/hw_api/attitude_cmd', HwApiAttitudeCmd, queue_size=10)
        self.pub_positions = rospy.Publisher('/uav3/predicted_positions', Float64MultiArray, queue_size=10)
        self.pub_slack_alpha = rospy.Publisher('/uav3/alpha_slack', Float64MultiArray, queue_size=10)
        # self.rate = rospy.Rate(10) #Hz
        self.data = None
        self.sub = rospy.Subscriber("/uav3/estimation_manager/uav_state", UavState, self.callback)
        self.sub_target = rospy.Subscriber("/uav1/target_estimate", Float64MultiArray, self.callback_target)
        self.sub_pos1 = rospy.Subscriber("/uav1/predicted_positions", Float64MultiArray, self.callback_positions1)
        self.sub_pos2 = rospy.Subscriber("/uav2/predicted_positions", Float64MultiArray, self.callback_positions2)
        ...
        if((rospy.get_time() - self.start_timer) > self.sim_time):
            ...
            if(rospy.get_time() - self.start_timer_2> 5):
                if(self.uav1_info and self.uav2_info):
                    ...
                    # self.dec_ocp.set_desired_angle(0.0)
        ...
        while not rospy.is_shutdown():
            if (self.data):
                ...
                if((self.iteration_counter > 3) and self.not_cmd_kill):
                    ...
                    command = "rosnode kill /uav3/control_manager"
                    try:
