...
class one_opt:
    ...
    def __init__(self):
        ...
        # Initialize ROS
        rospy.init_node('uav2_publisher_subscriber', anonymous=True) # Anonymous makes it so that multiple listeners can be run simultaneously, ensuring different names
        self.pub = rospy.Publisher('/uav2/hw_api/attitude_cmd', HwApiAttitudeCmd, queue_size=10)
        self.pub_positions = rospy.Publisher('/uav2/predicted_positions', Float64MultiArray, queue_size=10)
        self.pub_slack_alpha = rospy.Publisher('/uav2/alpha_slack', Float64MultiArray, queue_size=10)
        # self.rate = rospy.Rate(10) #Hz
        self.data = None
        self.sub = rospy.Subscriber("/uav2/estimation_manager/uav_state", UavState, self.callback)
        ...
        while not rospy.is_shutdown():
            if (self.data):
                ...
                if((self.iteration_counter > 3) and self.not_cmd_kill):
                    ...
                    command = "rosnode kill /uav2/control_manager"
                    try:
