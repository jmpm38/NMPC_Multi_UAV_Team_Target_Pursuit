...
# sim_number = 5
# sim_name = 'uav0/simulation_0_' + str(sim_number)

# b = bagreader(sim_name + '.bag')

# uav_state = b.message_by_topic('/uav1/mavros/local_position/odom')
# uav_state
# hw_cmd = b.message_by_topic('/uav1/mavros/setpoint_raw/target_attitude')
# hw_cmd
# target_info = b.message_by_topic('/uav1/target_estimate')
# target_info
# alpha_slack_info = b.message_by_topic('/uav1/alpha_slack')
# velocity_info = b.message_by_topic('/uav1/mavros/local_position/velocity_local')

# file = open(sim_name  + '/uav1-mavros-local_position-odom.csv')
# type(file)

# file2 = open(sim_name  + '/uav1-mavros-setpoint_raw-target_attitude.csv')
# type(file2)

# file3 = open(sim_name  + '/uav1-target_estimate.csv')
# type(file3)

# file4 = open(sim_name  + '/uav1-alpha_slack.csv')
# type(file4)

# file5 = open(sim_name  + '/uav1-mavros-local_position-velocity_local.csv')
# type(file5)
...
sim_number = 2
sim_name = 'sim_' + str(sim_number)

b = bagreader(sim_name + '.bag')

uav_state = b.message_by_topic('/mavros/local_position/odom')
uav_state
hw_cmd = b.message_by_topic('/mavros/setpoint_raw/attitude')
hw_cmd
target_info = b.message_by_topic('/uav1/target_estimate')
target_info
alpha_slack_info = b.message_by_topic('/uav1/alpha_slack')
velocity_info = b.message_by_topic('/mavros/local_position/velocity_local')

file = open(sim_name  + '/mavros-local_position-odom.csv')
type(file)

file2 = open(sim_name  + '/mavros-setpoint_raw-attitude.csv')
type(file2)
...
file5 = open(sim_name  + '/mavros-local_position-velocity_local.csv')
type(file5)
...
time_vec = rows[:, 0]
time_vec = time_vec.astype(float)
time_vec_alpha_slack = time_vec[:]

columns_to_extract = [0, 3, 4, 5] # S1, S2, S3
column_alpha = [0, 3] # Alpha_desired #6 -> 3 !!! because no alpha for single drone

slack_variables = rows[:, columns_to_extract]
slack_variables = slack_variables.astype(float)

alpha_desired = rows[:, column_alpha]
...
# columns_to_extract[0, 5, 6]

# time_vec = rows[:, 0]
# time_vec = time_vec.astype(float)
# time_vec_states = time_vec[:]

columns_to_extract = [5, 6, 7]
...
# print(target_positions_final.shape)
...
end_time = min([min(max(time) for time in time_rows), (start_time + sim_time)])
...
# graph.set_variables(target_positions_final[1:4, :],states[1:, :], controls[1:, :], controls.shape[1], states[0, :], slack_variables, alpha_desired, uav_number - 1, target_positions_final[0, :], controls[0, :], slack_variables[0, :],target_positions_final[4:6, :]) 
graph.set_variables(interpolated_arrays[2][1:4, :], interpolated_arrays[0][1:, :], interpolated_arrays[1][1:, :], controls.shape[1], interpolated_arrays[0][0, :] - interpolated_arrays[0][0, 0], interpolated_arrays[3][1:, :], interpolated_arrays[4][1:, :], uav_number - 1, interpolated_arrays[2][4:6, :])
# graph.set_variables(interpolated_arrays[2][1:4, :], states[1:, :], interpolated_arrays[1][1:, :], controls.shape[1], states[0, :], interpolated_arrays[3][1:, :], interpolated_arrays[4][1:, :], uav_number - 1, interpolated_arrays[2][4:6, :])
# graph.set_variables(interpolated_arrays[1][1:4, :], interpolated_arrays[0][1:, :], controls, controls.shape[1], interpolated_arrays[0][0, :] - interpolated_arrays[0][0, 0], interpolated_arrays[2][1:, :], interpolated_arrays[3][1:, :], uav_number - 1, interpolated_arrays[1][4:6, :])
...
# # Step 2: Determine common time axis
# start_time = max([min(time) for time in time_rows])
# end_time = min([min(max(time) for time in time_rows), sim_time])

# # Define the resolution of the common time axis (number of points)
# # You may want to set this to match the highest resolution in your data
# min_num_columns = min(arr.shape[1] for arr in arrays)
# common_time = np.linspace(start_time, end_time, num=min_num_columns)
...
# # PLOTS
# print("UAV: " + str(uav_number - 1))
# graph.set_variables(interpolated_arrays[2][1:4, :], interpolated_arrays[0][1:, :], interpolated_arrays[1][1:, :], controls.shape[1], interpolated_arrays[0][0, :], interpolated_arrays[3][1:, :], interpolated_arrays[4][1:, :], uav_number - 1, interpolated_arrays[2][4:6, :])
# graph.plot_starter()
