...
class plot_graphs_class:
    ...
    def yaml_handling(self):
            ...
            # package_path = rospack.get_path('real_life_tests')
    ...
    def fun_plot_uav_target_XY(self):
        ...
        for uav_i in range(0, self.uav_num * 9, 9):
            ...
            plt.scatter(self.X_states[0 + uav_i, :], self.X_states[1 + uav_i, :], label=('$p_{UAV}$'), s=10)
            # plt.quiver(self.X_states[0 + uav_i, :], self.X_states[1 + uav_i, :], u, v, angles='xy', scale_units='xy', scale=0.5, color='red')
            i += 1
        plt.plot(self.X_states[0 + uav_i, 0], self.X_states[1 + uav_i, 0], 'go', label = 'initial')
        plt.plot(self.X_states[0 + uav_i, -1], self.X_states[1 + uav_i, -1], 'co', label = 'final')

        # Plot target position
        plt.scatter(self.target_position[0, :], self.target_position[1, :], color='r', label="$p_T$", marker="*", s=10)
        plt.plot(self.target_position[0 + uav_i, 0], self.target_position[1 + uav_i, 0], 'gs', label = 'initial')
        plt.plot(self.target_position[0 + uav_i, -1], self.target_position[1 + uav_i, -1], 'cs', label = 'final')
        plt.grid()

        # Adjust the font size of subscripts (and other math text)
        # plt.rcParams['legend.fontsize'] = 12  # Adjust legend font size
        # plt.rcParams['mathtext.default'] = 'regular'  # Ensure subscripts are not too large
        # plt.rcParams['mathtext.fontset'] = 'stix'  # Optional: use STIX font set for better LaTeX rendering
        # plt.rcParams['mathtext.rm'] = 'Times'  # Optional: use Times Roman for normal text in math
        ...
        plt.xlim(-15, 30)  # Example: limits for x-axis
        plt.ylim(-15, 30)  # Example: limits for y-axis
    ...
    def fun_plot_uav_xyz(self):
        ...
        for uav_i in range(0, self.uav_num*9, 9):
            ...
            # plt.plot(self.time_vec_state, self.target_position[0, :], label=('X_t' + str(i)))
            # plt.plot(self.time_vec_state, setime_stepslf.target_position[1, :], label=('Y_t' + str(i)))
    ...
    def fun_plot_uav_yaw_yaw_ref(self):
        ...
        time_vec_aux = np.insert(self.time_vec_control, np.where(mask)[0] + 1, np.nan)
    ...
    def fun_plot_target_vxvy(self):
        ...
        plt.plot(self.time_vec_target, self.target_speed[0, :], label='$v_{x,T}$')
        plt.plot(self.time_vec_target, self.target_speed[1, :], label='$v_{y,T}$')
        plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
        plt.tight_layout(rect = [0.05, 0.05, 1, 0.95])
        plt.ylabel('Velocity (m/s)')
        plt.xlabel('Time (s)')
    ...
    def plot_slack_variables(self):
        ...
        plt.plot(self.time_vec_slack, self.slack_variables[0, :], label=('$S_1$'))
        plt.plot(self.time_vec_slack, self.slack_variables[1, :], label=('$S_2$'))
        plt.plot(self.time_vec_slack, self.slack_variables[2, :], label=('$S_3$'))
    ...
    def plot_alpha_vs_desired_alpha(self):
        ...
        plt.plot(self.time_vec, alpha_ref, label=('alpha_ref'))#, marker = ':')
        plt.plot(self.time_vec, alpha_angle, label=('alpha'))#, marker=':')
        plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
        plt.tight_layout(rect=[0, 0, 1, 0.95])
    ...
    def set_uavs_positions(self, uav_i_states, uav_i):
        ...
        if(uav_i == 2):
            ...
            max_columns = max(arr.shape[1] for arr in arrays)
    ...
    def plot_angular_separation(self):
        self.separation_angles = np.zeros((math.comb(3,2), self.time_steps))
        for k in range(self.time_steps):
            iterator = 0
        ...
        for uav_i in range(0, self.real_uav_num, 1):
            for uav_j in range(uav_i + 1, self.real_uav_num, 1):
                plt.plot(self.time_vec, self.separation_angles[iterator, :], label=('angular difference, uavs ' + str(uav_i + 1) + ' ' + str(uav_j + 1)))
                iterator = iterator + 1
        plt.legend()
        plt.axhline(y=0, color='r', linestyle=':')
        ...
        plt.ylabel('Angle (deg)')
        plt.xlabel('Time (s)')
        plt.title("Angular Separation between UAVs")
    ...
    def plot_solving_time(self, solving_times):
        ...
        plt.plot(solving_times[0, :] - solving_times[0, 0], solving_times[1, :])#, marker = ':')
